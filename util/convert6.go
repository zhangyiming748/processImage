...
/*
 */
func Static6(in GetFileInfo.Info, threads string) string {
	...
	cmd := exec.Command("ffmpeg6", "-threads", threads, "-i", in.FullPath, "-c:v", "libaom-av1", "-still-picture", "1", "-threads", threads, out)
...
返回输出文件全路径
*/
func Dynamic6(in GetFileInfo.Info, threads string) {
	prefix := strings.Trim(in.FullPath, in.ExtName)
	out := strings.Join([]string{prefix, "avif"}, ".")
	cmd := exec.Command("ffmpeg6", "-threads", threads, "-i", in.FullPath, "-threads", threads, out)
	// Todo 如果转换出现错误 记录最后一个出问题的文件
	defer func() {
		if err := recover(); err != nil {
			log.Debug.Printf("最后出问题的源文件%v\t目标文件%v\n", in.FullPath, out)
